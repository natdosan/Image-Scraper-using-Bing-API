...
function download(url, image_path) { //https://stackoverflow.com/questions/12740659/downloading-images-with-node-js
    return axios({
        url, //url: url,
        ...
        response =>
            new Promise((resolve, reject) => {
                response.data
                    .pipe(fs.createWriteStream(image_path)) //watch video
                    .on('finish', () => resolve())
                    .on('error', e => reject(e));
            }),
    );
...
function getBingData(query, start, count) {
    ...
    return axios({ //object
        ...
        params: { //all params of the query
            q: query,
            offset: start, //dynnamic offset
            size: "Large",
            count: count,
        },
        headers: {
...
//getImages("IU cute face picture").then((filesArray) => {
    ...
    return new Promise((resolve, reject) => {
        ...
        setTimeout(function () {
            ...
            if (totalPhotosRequested - filesArray.length >= 150) {
                ...
                resolve(getImages(query, start + 150, totalPhotosRequested - filesArray.length)) //recurser function because it runs until a condition is satisfied
            } 
        }, 3000); //settingthe waitinng time to 3 seconds; after the time, we want to execute the functionn inside it (1st param)
    })
}
...
function getAndSaveImages(query, numberOfPhotos) {
    getImages(query, 0, numberOfPhotos).then(async (filesArray) => {
        ...
        for (i = 0; i < filesArray.length; i++) {
            ...
            if ((/\.(jpg|JPG|jpeg|JPEG|png|PNG|gif|GIF|webp|WEBP|jfif|JFIF)$/g).test(dowloadedImg)) {
                download(filesArray[i], path.join(__dirname, "downloaded", dowloadedImg)).then(() => {
                    console.log(`downloaded photo ${dowloadedImg}`)
                }).catch(() => {
                    console.log(`skipped photo ${dowloadedImg}`)
                })
            } else {
                console.log(`skipped photo ${dowloadedImg} because there is no extension`)
            }
        }
    }
    )
